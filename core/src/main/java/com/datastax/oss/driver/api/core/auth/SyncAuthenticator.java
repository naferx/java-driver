...
public interface SyncAuthenticator extends Authenticator {
   ...
   *     driver will <b>clear the contents</b> of the returned byte buffer immediately after use (to
   *     avoid keeping sensitive information in memory).
   */
  @Nullable
   ...
   *     further action). Note that the driver will <b>clear the contents</b> of the returned byte
   *     buffer immediately after use (to avoid keeping sensitive information in memory); if your
   *     authenticator returns the same result for multiple evaluations, do not reuse the same
   *     buffer.
   */
  @Nullable
