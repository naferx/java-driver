 ...
 * a partial primary key, all other parameters must be explicitly annotated with {@link
 * CqlName}.</b> For example if the primary key is {@code ((day int, hour int, minute int), ts
 * timestamp)}:
 *
 * <pre>
...
public @interface Select {
   ...
   *
   * <p>Each element must be a column name followed by a space and the word "ASC" or "DESC".
   *
